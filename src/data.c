...
Laik_Data *laik_new_data(Laik_Space *space, Laik_Type *type)
{
    ...
    d->layout = LAIK_Lex_Layout;             // by default, use lex layouts
...
void laik_data_set_layout_flag(Laik_Data *d, Laik_Use_Layout_t layout)
{
    d->layout = layout;
}
...
// this is for the lex layout
static Laik_Range *coveringRanges_lex_l(int n, Laik_RangeList *list, int myid)
{
    if (n == 0)
        return 0;
...
Laik_Range *coveringRanges_vector_l(Laik_RangeList *list, int myid, uint64_t * map_size)
{
    ...
    Laik_Range *range = (Laik_Range *)malloc(sizeof(Laik_Range));

    laik_log(1, "coveringRanges: 1 map");
...
static Laik_MappingList *prepareMaps(Laik_Data *d, Laik_Partitioning *p)
{
    if (!p)
        return 0; // without partitioning borders there are no mappings
...
static uint64_t freeMap(Laik_Mapping *m, Laik_Data *d, Laik_SwitchStat *ss)
{
    assert(d == m->data);
...
// return number of bytes freed
static uint64_t freeMappingList(Laik_MappingList *ml, Laik_SwitchStat *ss)
{
    if (ml == 0)
        return 0;
...
static void laik_map_set_allocation(Laik_Mapping *m,
...
static void copyMaps(Laik_Transition *t,
                     ...
                     Laik_SwitchStat *ss)
{
    ...
    if ((fromList->res != 0) && (fromList->res == toList->res))
        return;
...
static void initEmbeddedMapping(Laik_Mapping *toMap, Laik_Mapping *fromMap)
{
...
static void initMaps(Laik_Transition *t,
                     Laik_MappingList *toList, Laik_MappingList *fromList,
                     Laik_SwitchStat *ss)
{
...
static void allocateMappings(Laik_MappingList *toList, Laik_SwitchStat *ss)
{
    for (int i = 0; i < toList->count; i++)
    {
...
static Laik_ActionSeq *createTransASeq(Laik_Data *d, Laik_Transition *t,
...
static void doTransition(Laik_Data *d, Laik_Transition *t, Laik_ActionSeq *as,
                         Laik_MappingList *fromList, Laik_MappingList *toList)
{
    ...
    if (as)
    {
        ...
        if (tc->prepFromList)
            assert(tc->prepFromList == fromList);
        if (tc->prepToList)
            assert(tc->prepToList == toList);
    }
    else
    {
...
static int mygroup_cmp(const void *p1, const void *p2)
{
...
void laik_reservation_alloc(Laik_Reservation *res)
{
    ...
    for (int i = 0; i < mCount; i++)
    {
        ...
        m->layout = (data->layout_factory)(1, range, 0);
        m->layoutSection = 0;
...
static void checkOwnParticipation(Laik_Data *d)
{
...
Laik_Mapping *laik_global2local_1d(Laik_Data *d, int64_t gidx, uint64_t *lidx)
{
    ...
    for (int i = 0; i < d->activeMappings->count; i++)
    {
        ...
        if (gidx < m->requiredRange.from.i[0])
            continue;
        if (gidx >= m->requiredRange.to.i[0])
            continue;

        if (lidx)
            *lidx = gidx - m->requiredRange.from.i[0];
        return m;
    }
    return 0;
}
